...
import loveletter.src.model.cards.*;
import loveletter.src.model.players.LearningRobot2;
import loveletter.src.model.players.Player;
import loveletter.src.model.players.Robot1;
import loveletter.src.database.DatabaseQueries;
...
public class Game {
	...
	/**
	 * When the round is over, determine the winner and give him token
	 */
	private void end() {
	...
	/**
	 * When the game is over, determine the game winner
	 */
	private void endGame() {
		...
		this.printGameStatus();
		this.winnerGame = this.players.getFirst();
		for (Player p : this.players) {
			if (!p.equals(this.winnerGame)) {
				if (p.getNbToken() > this.winnerGame.getNbToken()) {
					hasEquality = false;
					this.winnerGame = p;
				} else if (p.getNbToken() == this.winnerGame.getNbToken()) {
					hasEquality = true;
				}
			}
...
//		this.players.add(new LearningRobot(this));
		this.players.add(new LearningRobot2(this));
	...
	public void setWinner() {
		if (this.isOver()) { //Si tous les joueurs ont perdus sauf un
			for (Player p : this.players) {
				if (!p.hasLoose()) {
					this.winner = p;
	...
	public boolean hasWinnerInEnd() {
		this.winner = this.players.getFirst();
		
		for (Player p : this.players) {
			if (!winner.equals(p)) { //Si ce n'est pas  le même joueur
				try {
					Card cardPlayer = p.getFirstCard();
					Card cardWinner = winner.getFirstCard();
					if (!cardPlayer.isEquals(cardWinner) ) { //Si leur cartes ne sont pas égales
						//Si p a une meilleur carte que winner
						if (!p.hasCards() && !winner.hasCards() && cardPlayer.isGreater(cardWinner))
							this.winner = p;
	...
	/**
	 * Give a token to the player who has put a spy and doesn't lose
	 */
	...
	/**
	 * Give a token to the player who has win
	 */
